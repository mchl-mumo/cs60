...
class SpampedeView {
	...
	public void updateGraphics() {
		...
		int high = Preferences.NUM_CELLS_TALL;
		int wide = Preferences.NUM_CELLS_WIDE;
		for (int row = 0; row < high; row++) {
			for (int column = 0; column < wide; column++) {
				Color cellColor = model.getCellColor(row, column);
				this.drawSquare(column*Preferences.CELL_SIZE, row*Preferences.CELL_SIZE, cellColor);
			}
		}
